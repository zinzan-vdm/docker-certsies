...
export PROVIDER=${LEXICON_PROVIDER-cloudflare}
...
function deploy_challenge {
  ...
  lexicon --config-dir=/certsies "$PROVIDER" create ${DOMAIN} TXT --name="_acme-challenge.${DOMAIN}." --content="${TOKEN_VALUE}"
...
function clean_challenge {
  ...
  lexicon --config-dir=/certsies "$PROVIDER" delete ${DOMAIN} TXT --name="_acme-challenge.${DOMAIN}." --content="${TOKEN_VALUE}"
...
function deploy_cert {
  ...
  chmod 'u=rw,g=r,o=' "${KEYFILE}" "${CERTFILE}" "${FULLCHAINFILE}" "${CHAINFILE}" "$(dirname ${CERTFILE})/combined.pem"
